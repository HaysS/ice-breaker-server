...
exports.getUser = function (key) {
  return firebase.database().ref().child('users').child(key).once('value').then(snap => snap.val());
};

exports.getUserCb = function (key, func) {
  firebase.database().ref().child('users').child(key).once('value').then(snap => func(snap.val()));
};
